...
We had to make some design decisions for our inference provider, embedding model, and chat model. We found 3 inference provider abstractions from our research: GroqCloud, OpenRouter, and Vertex AI.
...
Ultimately, we settled on OpenRouter as its model support provides the greatest developmental flexibility.
For our embedding model, we wanted a model with a context size greater than 512 tokens for input flexibility.
Sorting the models officially supported by OpenRouter with that property by price, the top 3 models were OpenAI's Text Embedding 3 Small (TE3-Small), BAAI's BGE-M3 (BGE-M3), and Qwen's Qwen3 Embedding 8B (QE-8B).
...
Ultimately, we settled on QE-8B as the higher context size gives input flexibility, and the higher price adds negligible cost.
...
a context size no less than 128,000 tokens for in-conversation memory, input modalities including text and image for input flexibility, and a maximum output size no less than 8,000 tokens for output flexibility.
Sorting the models officially supported by OpenRouter with that property by price, the top 3 models were OpenAI's GPT 5 Chat (gpt-5-chat), Google's Gemini 2.5 Flash (gemini-2.5-flash), and Meta's Llama 4 Maverick (llama-4-maverick).
...
Ultimately, we kept these 3 models as candidates for more rigorous downstream evaluative ablations to decide model deployment, following the industry practice of architecting systems that facilitate model switching \parencite{ng2025}.
...
Anonymous access requires no registration and lets users ask questions immediately.
Authenticated access requires registration and lets accounts retain preferences and past conversations.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.95\linewidth]{images/frontend-full.png}
	\caption{Low-fidelity sketches of frontend UI.}
\end{figure}
...
The frontend is implemented with Next.js 15 using a modular component architecture.
Routing and layout reside in the main application layer, while reusable interface elements are factored into separate components.
...
Configuration is driven by environment variables that provide the backend application programming interface (API) base uniform resource locator (URL) and related parameters.
...
We found 3 cloud provider abstractions from our research: Amazon Web Services (AWS), Microsoft Azure (Azure), and Google Cloud Platform (GCP).
...
Within AWS, we found 6 compute service abstractions from our research: Elastic Compute Cloud (EC2), Lambda, Fargate, App Runner, Lightsail, and Elastic Beanstalk.
...
To handle dynamic traffic, integrate with autoscaling, and support future features, Fargate was chosen as the primary compute layer, allowing the number of running tasks to grow or shrink with demand.
