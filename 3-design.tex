...
\begin{enumerate}
	...
	\item \textbf{Indexing.} The system stores each chunk and its corresponding embedding in a database optimised for embedding retrieval.
...
We had to make several design decisions for our inference provider, embedding model, and chat model.
...
Ultimately, we kept these three models as candidates for more rigorous downstream evaluative ablations to decide model deployment, following the industry practice of architecting systems that facilitate model switching \parencite{ng2025}.
...
\begin{figure}[H]
	...
	\caption{Low-fidelity sketches of the frontend UI.}
\end{figure}
...
Here, virtual central processing unit (vCPU) hours and gigabyte (GB) hours measure compute and memory usage, while load balancer capacity unit (LCU) hours are used for managed load balancers. To handle dynamic traffic, integrate with autoscaling, and support future features, AWS App Runner was chosen as the primary deployment service, allowing the number of running instances to grow or shrink with demand.
