...
In Iteration 1, we created our first prototype: Prototype 1.
...
It implemented the computational graph correctly on the backend, and rendered the output of the chat model without token artifacts on the frontend.
...
In Iteration 2, we expanded Prototype 1 with a set of core functional upgrades, to create our second prototype: Prototype 2.
...
Prototype 2 introduced several user-facing improvements.
...
Chats could also be exported as PDF files, and past conversations were stored in the browser through LocalStorage.
...
\begin{figure}[H]
	...
	\caption{Diagram of planned system architecture.}
\end{figure}

\vspacebaselineskip

A continuous integration and deployment (CI/CD) pipeline will be introduced using GitHub Actions. Each push to the main branch will trigger automated tests, build container images, publish them to the registry, and update the ECS services running on Fargate. This ensures that only validated builds are deployed and reduces manual operational overhead while maintaining consistency between the codebase and the live environment.

\begin{figure}[H]
	\centering
	\includegraphics[width=0.95\textwidth]{images/timeline.png}
	\caption{Diagram of planned project timeline.}
\end{figure}
