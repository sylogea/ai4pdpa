...
Three iterations improved UI design, retrieval quality, safety behaviour, and early features such as conversation storage, PDF export, and refusal logic.
Benchmarking of three LLM candidates was conducted using a preliminary RAG pipeline to reduce hallucination rates.

From an engineering standpoint, Term 7 included setting up the cloud deployment pathway, comparing AWS compute options, and implementing a basic CI workflow.
...
Several insights from Term 7 informed the priorities for Term 8. First, retrieval-based grounding is essential for accuracy. Early tests showed that even strong models occasionally mixed PDPA with foreign regulations such as the GDPR, underscoring the need for a tightly scoped retrieval corpus and jurisdiction-specific prompting. This guided both our model-selection criteria and the structure of the RAG pipeline.
...
This shaped decisions around architecture, storage, and sequencing of development.
...
Finally, AWS exploration showed that managed compute services such as Fargate offer a good balance of performance, scalability, and operational simplicity.
This became the foundation for the Term 8 deployment plan.
Together, these insights provide a focused direction for completing the platform in the next phase.
...
Based on the current architecture, AWS infrastructure costs amount to approximately SGD 108 per month, covering Fargate compute, storage, load balancing, and operational monitoring.
...
These costs reflect real deployment behaviour observed during Term 7 testing and will be refined as additional Term 8 features come online.
...
% A detailed cost breakdown is found in Appendix E.
...
Key technical risks included API rate limits, model hallucination, cloud misconfiguration, and retrieval errors, all of which were mitigated through IAM reviews, billing alerts, retrieval constraints, and controlled refusal behaviour.
...
Term 7 established the foundation for AI4PDPA by validating the problem context, consolidating requirements with AsiaCloud, and developing the first end-to-end prototype of the platform.
